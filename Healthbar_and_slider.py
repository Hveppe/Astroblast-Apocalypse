...
class Slider:  # slider til forskellige ting
    ...
    def draw(self, x, y):
        self.rect = pygame.draw.rect(self.screen, self.color_square, (x, y, self.width-10*scalar, self.height))

        if self.sliderrect_x is None:
            self.sliderrect_x = x+self.width-10*scalar
        ...
        if self.rect.collidepoint(mouse_pos):
            if pygame.mouse.get_pressed()[0]:
                ...
                self.return_data = (self.sliderrect_x / (self.width - 10 * scalar)) * self.max_number
